...
class Block(Element):
    ...
    def export_data(self):
        ...
        if(self.get_key() == "variable_es_queue"):
            n['event_binding'] = self.export_bindings();
    ...
    def import_data(self, n):
        ...
        while get_hash() != my_hash:
            ...
            bindings_n = n.findall("event_binding");
            if(len(bindings_n) > 0):
                self._bindings = list();
                for binding_n in bindings_n:
                    emitter = binding_n.find("emitter");
                    handler = binding_n.find("handler");
                    self._bindings.append( [emitter,handler] );

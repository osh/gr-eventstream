...
class Block(_Block, _GUIBlock):
	...
	def __init__(self, flow_graph, n):
		...
		self._event_emit = map(lambda i: i.strip(), n.findall('event_emit'))
		self._event_handle = map(lambda i: i.strip(), n.findall('event_handle'))
	...
	def events_emitted(self):
		eve = [];
		for v in self._event_emit:
            # if definition is a variable evaluate it
			if re.match(r"\$[\w\d]+", v):
				varname = v.strip()[1:];
				try:
					v = self.get_param(varname).get_value();
				except:
					print "WARNING: emit variable %s could not be resolved to a parameter!"%(v);
			eve.append(v);
		return eve;

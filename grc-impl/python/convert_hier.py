...
def convert_hier(flow_graph, python_file):
    ...
    # define any events that may be emitted by this block
    if(flow_graph._suppress_io): #_suppress_io = is evt handler
        block_n['event_emit'] = flow_graph.get_events_generated();

    # define as event handler
    if(flow_graph._suppress_io): #_suppress_io = is evt handler

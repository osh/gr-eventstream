...
class FlowGraph(_FlowGraph, _GUIFlowGraph):
    ...
    def _eval(self, code, namespace, namespace_hash):
        ...
        if not self._eval_cache.has_key(my_hash):
            try:
                ...
                print "EXCEPTION DURRING EVAL: %s\n%s"%(code, sys.exc_info()[0]);
        #return from cache
    ...
    def get_events_generated(self):
        print "get_events_generated"
        evts = [];
        for b in self.get_enabled_blocks():
            if b.get_key() == "es_hier_event_emit":
                evts.append(b.get_param("evtname").get_value());
        return evts;

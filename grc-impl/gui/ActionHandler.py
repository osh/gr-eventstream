...
from BindingsDialog import BindingsDialog
import Dialogs
        ...
        ##################################################
            ...
            if selected_block:
                if PropsDialog(selected_block).run():
                    ...
                    self.get_flow_graph().update()
        ##################################################
        # Event Bindings Modify
        ##################################################
        elif action == Actions.BLOCK_BINDINGS:
#            print "got BLOCK BINDINGS"
            selected_block = self.get_flow_graph().get_selected_block()
            if selected_block:
                if BindingsDialog(self.get_flow_graph(), selected_block).run():
                    #save the new state
                    self.get_flow_graph().update()
                    self.get_page().get_state_cache().save_new_state(self.get_flow_graph().export_data())
                    self.get_page().set_saved(False)
#                    print "bindings dlg returned true"
                else:
                    #restore the current state
                    n = self.get_page().get_state_cache().get_current_state()
                    self.get_flow_graph().import_data(n)
                    self.get_flow_graph().update()
#                    print "bindings dlg returned false"
        ...
        if(len( self.get_flow_graph().get_selected_blocks()) == 1):
            Actions.BLOCK_BINDINGS.set_sensitive( self.get_flow_graph().get_selected_blocks()[0]._key == "variable_es_queue" );
        else:
            Actions.BLOCK_BINDINGS.set_sensitive(False)

...
class EntryParamML(InputParam):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._input.set_size_request(300, 250)
        self._input.set_editable(True);
        self._input.get_buffer().set_text(  self.param.get_value()[2:-2] );
        self._input.get_buffer().connect('changed', self._handle_changed)
        self.pack_start(self._input, True)
    def get_text(self): 
        buf = self._input.get_buffer();
        return '""'+str(buf.get_text( buf.get_start_iter(), buf.get_end_iter() ))+' ""';
#        return str(buf.get_text( buf.get_start_iter(), buf.get_end_iter() ));
    def set_color(self, color): pass
    def set_tooltip_text(self, text): pass
    # TODO: fix these for gtk.Text()
    #def set_color(self, color): self._input.modify_base(gtk.STATE_NORMAL, gtk.gdk.color_parse(color))
    #def set_tooltip_text(self, text): self._input.set_tooltip_text(text)
...
class Param(Element):
    ...
    def get_input(self, *args, **kwargs):
        ...
        if self.is_multiline(): return EntryParamML(self, *args, **kwargs)
